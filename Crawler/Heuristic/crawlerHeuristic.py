...
#condicoes heuristicas utilizadas

def codicoes_heuristicas(link_final, dominio):
        positives=["blusa","blusinhas","blus","t-shirt","tshirt"]
        negatives=["calça","short","jeans","saia","vestido","biquine"]
        score = 0.4
        if(dominio not in link_final):
            print("entrei")
            score = 0
        elif(dominio in link_final):
            print("entrei sim")
            for positive in positives:
                if(positive in link_final):
                    score = 1
                    for negative in negatives :
                        if( negative in link_final):
                            score = 0.7
                else:
                    score = 0.5
        return score
    ...
    def bsfCrawl(self):
        ...
        while( len(self.valid_links) < self.maxPages):
            ...
            for a in crawler: # para todos filhos crawlei
                ...
                score = codicoes_heuristicas(link_final,url_original)
                print(link_final)
                print(score)
                ...
                elif(link_final.find('whatsapp://send?text')==0):
                    if(link_final not in self.invalids):
                        self.invalids.append(link_final)

                elif( score >= 0.5 and autorizado and link_final not in visited):
                    self.valid_links.append(link_final)
                    visited.append(link_final)
                    # adicionando no array um link valido
            url_atual = self.valid_links[count]
            count = count + 1
            self.loops = count
